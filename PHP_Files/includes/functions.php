...
function check_pass($reg,$pass){
    $query="select RegNumber from bus WHERE RegNumber='{$reg}' AND password='{$pass}'";
    $result=exicute_query($query);
...
function CheckUser($userName,$password){
    alert("chekin user 123");
    $result = exicute_query("select ID,Name from busowner WHERE UserName='$userName' AND Password='$password' limit 1");
...
function check_Authantication($userName,$password){
    alert("Authanticate {$userName} {$password}");
    $result = exicute_query("select * from user WHERE UserName='$userName' AND Password='$password' limit 1");
    if(!$user=mysqli_fetch_array($result)){
        return false;
    }
